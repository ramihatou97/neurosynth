...
def run_search_and_extract():
    ...
    for pdf_path in all_pdfs:
        ...
        if not file_matches:
            print(f"  ⚠️ Could not find query '{query}' in {pdf_path.name}. Trying 'lumbar'...")
            file_matches = searcher.search_pdf("lumbar", pdf_path)
            
        if not file_matches:
            print(f"  ⚠️ Could not find query 'lumbar' in {pdf_path.name}. Trying 'spine'...")
...
async def run_synthesis():
    ...
    env = os.environ.copy()
    env["PYTHONPATH"] = f"{project_root}/src:{project_root}/reference-library:{env.get('PYTHONPATH', '')}"
    
    cmd = [
        python_exe, "-m", "neurosynth.cli", "run",
        "Lumbar Microdiskectomy",
        "--sources", str(SOURCES_DIR),
        "--manifest", str(MANIFEST_PATH),
        "--output", str(SYNTHESIS_DIR)
    ]
