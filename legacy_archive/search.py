...
class SearchEngine:
    ...
    def __init__(self, db: Database):
        self.db = db
        self._chunk_cache: list[tuple[Chunk, list[float]]] | None = None
        self._image_cache: list[tuple[ExtractedImage, list[float]]] | None = None
    ...
    def search_images(
        ...
        top_k: int = 20,
        min_score: float = 0.5,
        image_types: list[ImageType] | None = None,
        source_ids: list[str] | None = None,
    ...
    def retrieve_for_topic(
        ...
        # Only diversify if NOT strictly filtering by a small set of sources
        if not source_ids or len(source_ids) > 1:
            diversified = self._diversify_sources(chunk_results, top_k)
        else:
            # If strictly filtering for 1 source, just return best results
            diversified = chunk_results[:top_k]
        ...
        if include_images:
            ...
            image_results = self.search_images(
                query_embedding=query_embedding, top_k=50, source_ids=list(sources_used)
            )
    ...
    def _diversify_sources(
        self, results: list[SearchResult], target_count: int, max_per_source: int = 10
    ) -> list[SearchResult]:
        """
    ...
    def get_chunks_by_type(
        self, chunk_type: ChunkType, limit: int = 100
    ) -> list[Chunk]:
