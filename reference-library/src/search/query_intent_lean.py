...
class IntentDetector:
    ...
    INTENT_KEYWORDS: Dict[QueryIntent, Set[str]] = {
        QueryIntent.TECHNIQUE: {
            "technique", "techniques", "procedure", "procedures", "steps",
            "operative", "surgery", "surgical", "approach", "approaches",
            "how to", "method", "methods"
        },
        QueryIntent.COMPLICATION: {
            "complication", "complications", "risk", "risks", "avoidance",
            "prevention", "management", "safety", "pitfalls", "hazards",
            "adverse events", "failure"
        },
        QueryIntent.ANATOMY: {
            "anatomy", "anatomical", "structure", "structures", "course",
            "relationship", "relationships", "nerve", "artery", "vein",
            "location", "position"
        },
        QueryIntent.INDICATION: {
            "indication", "indications", "contraindication", "contraindications",
            "selection", "patient selection", "criteria", "when to", "decision"
        },
        QueryIntent.OUTCOME: {
            "outcome", "outcomes", "prognosis", "results", "survival",
            "recurrence", "recovery", "follow-up"
        }
    }
    ...
    STRONG_KEYWORDS: Dict[QueryIntent, Set[str]] = {
        QueryIntent.TECHNIQUE: {
            "how to", "surgical steps", "operative steps"
        },
        QueryIntent.COMPLICATION: {
            "complication", "complications", "pitfall", "pitfalls", 
            "morbidity", "mortality", "adverse event", "adverse events"
        },
        QueryIntent.INDICATION: {
            "indication", "indications", "contraindication", "contraindications",
            "patient selection", "selection criteria", "when to"
        },
        QueryIntent.ANATOMY: {
            "anatomy", "anatomical", "landmark", "landmarks"
        },
        QueryIntent.OUTCOME: {
            "outcome", "outcomes", "prognosis", "survival rate"
        }
    }
    ...
    SECTION_MAP: Dict[QueryIntent, List[str]] = {
        QueryIntent.TECHNIQUE: [
            ...
            "Surgical Steps", "Operative Nuances", "Technique"
        ],
        QueryIntent.COMPLICATION: [
            "Complications", "Complication Avoidance", "Postoperative Management",
            "Avoidance of Complications", "Risks"
        ],
        QueryIntent.ANATOMY: [
            "Surgical Anatomy", "Anatomy", "Relevant Anatomy",
            "Anatomical Considerations"
        ],
        QueryIntent.INDICATION: [
            "Indications", "Patient Selection", "Clinical Presentation",
            "Contraindications", "Decision Making"
        ],
        QueryIntent.OUTCOME: [
            "Outcomes", "Results", "Prognosis", "Long-term Results"
        ]
    }
        ...
        for intent, keywords in self.STRONG_KEYWORDS.items():
            for keyword in keywords:
                ...
                if re.search(pattern, query_lower):
                    ...
                    if not cleaned: cleaned = query # Fallback if everything removed
