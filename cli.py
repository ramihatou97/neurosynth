...
def ingest(library_path: Optional[str], force: bool):
    ...
    if not path.exists():
        ...
        console.print(
            "Create the directory and add your PDFs, or specify a different path."
        )
        return
            ...
            try:
                ...
                if not force and db.source_exists(doc_id):
                    progress.update(
                        task,
                        advance=1,
                        description=f"Skipping {pdf_path.name} (already indexed)",
                    )
                    continue
                ...
                chunk_texts = [
                    c.content[:2000] for c in chunks
                ...
                if document.images:
                    progress.update(
                        task, description=f"Embedding {len(document.images)} images..."
                    )
                    img_contexts = [
                        f"{img.caption} {img.surrounding_text}"[:1000]
...
def synthesize(topic: str, template: str, output: Optional[str], format: str):
    ...
    # Summary
    console.print()
    console.print("[green]✓ Chapter synthesized successfully![/green]")
    console.print()
...
def search(query: str, limit: int, type: str):
    ...
    results = search_engine.search_chunks(
        query_embedding=query_embedding, top_k=limit, chunk_types=chunk_types
    )
...
def consult(question: str, verbose: bool):
    ...
    result = precision_search.search(
        query=question, query_embedding=query_embedding, top_k=10, include_images=False
    )

    # Show query classification
    console.print(
        f"📊 [dim]Type: {result.query_type.value} | Systems: {', '.join(result.systems_used)} | {result.retrieval_time_ms:.0f}ms[/dim]"
    )
    ...
    console.print(
        f"🎯 [bold {conf_color}]Confidence: {result.confidence:.0%} ({result.confidence_level.value})[/bold {conf_color}]"
    )
    ...
    for i, r in enumerate(result.results[:5], 1):
        score_display = (
            f"ColBERT: {r.colbert_score:.1f}"
            if r.colbert_score
            else f"Dense: {r.dense_score:.2f}"
        )
