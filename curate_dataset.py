...
def main():
    ...
    for p in [
        ...
        "contraindication_candidates.json",
        "comparative_candidates.json",
    ]:
        pp = Path(p)
        if pp.exists():
            with open(pp) as f:
    ...
    for qtype, target in TARGETS.items():
        ...
        if needed > 0:
            candidates = [
                q for q in buckets[qtype] if q["query"] not in selected_queries
            ]
            # Take needed
    ...
    output = {
        "version": "1.0.0",
        "metadata": {"total_queries": len(final_selection), "generated": True},
