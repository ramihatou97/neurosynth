...
class CaptionPatternMatcher:
    ...
    def _compile_patterns(self):
        ...
        self.figure_patterns = [
            ...
            for p in self.config.figure_patterns
        ]
        ...
        self.xref_patterns = [
            re.compile(p, re.IGNORECASE)
            for p in self.config.cross_reference_patterns
        ]
    ...
    def extract_figure_id(self, text: str) -> Tuple[Optional[str], str]:
        """
    ...
    def extract_step_number(self, text: str) -> Optional[int]:
...
class EnhancedCaptionDetector:
    ...
    def find_caption_for_image(
        ...
        for region in search_regions:
            ...
            for block in blocks:
                ...
                if caption:
                    ...
                    candidates.append(CaptionMatch(
                        caption=caption,
                        distance=distance,
        ...
        if best:
            ...
            best.confidence = self._calculate_confidence(
                best, image_bbox, candidates
            )
    ...
    def _get_search_regions(
        ...
        priorities = cfg.direction_priorities.get(position_key, [
        ...
        for priority, direction in enumerate(priorities):
            rect = self._create_search_rect(
                image_bbox, direction, page.rect, cfg
            )
            if rect:
    ...
    def _calculate_distance(
        ...
        import math
        return math.sqrt(
            (img_center_x - cap_center_x) ** 2 +
            (img_center_y - cap_center_y) ** 2
        )
...
class CrossReferenceTracker:
    ...
    def get_figure_location(self, figure_id: str) -> Optional[Tuple[int, tuple]]:
