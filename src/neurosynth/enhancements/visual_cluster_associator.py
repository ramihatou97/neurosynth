...
class NeurosurgicalKeywordScorer:
    ...
    def __init__(self, config: NeuroSynthEnhancedConfig = None):
        ...
        for kw in self._keyword_to_category.keys():
            if ' ' in kw:
    ...
    def score_text(
        ...
        details = {
            ...
            'raw_score_before_context': score / (
                self.weights.caption_context_boost if context_type == "caption"
                else self.weights.heading_context_boost if context_type == "heading"
                else 1.0
            ),
        }
...
class ContextAnalyzer:
    ...
    def __init__(self):
        ...
        self.demonstrative_patterns = [
            re.compile(r'\b(this|these|the following|above|below)\s+(?:image|figure|diagram|photo)', re.I),
    ...
    def analyze_context(
        self,
        text: str,
        image_figure_id: Optional[str] = None
    ) -> Tuple[float, Dict[str, Any]]:
        """
...
class EnhancedVisualClusterAssociator:
    ...
    def associate_all_on_page(
        ...
        for image in page_images:
            results[image.image_id] = self.associate_image_with_text(
                image, page_texts
            )
    ...
    def explain_score(self, score: AssociationScore) -> str:
        ...
        lines = [
            ...
            f"",
            f"Components (weights from config):",
...
class ClusterBuilder:
    ...
    def build_clusters(
        ...
        sorted_images = sorted(
            images,
            key=lambda i: (i.page_number, i.bbox[1])
        )
        ...
        for i, image in enumerate(sorted_images):
            ...
            good_assocs = [
                ...
                if a.score.total_score >= min_cluster_score
            ]
