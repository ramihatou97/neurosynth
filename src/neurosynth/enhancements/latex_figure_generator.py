...
class FigureImage:
    ...
    # Optional metadata
    subfigure_id: str | None = None  # (a), (b), etc.
...
class SubfigureBuilder:
    ...
    def build(self, spec: FigureSpec) -> str:
        ...
        for i, image in enumerate(spec.images):
            # Start subfigure
            lines.append(
                f"    \\begin{{subfigure}}[b]{{{subfig_width:.2f}\\textwidth}}"
            )
...
class ProceduralSequenceBuilder:
    ...
    def build(self, spec: FigureSpec) -> tuple[str, int]:
        ...
        for page_idx, page_images in enumerate(pages):
            ...
            latex = self._build_page(
                page_images, spec, page_idx, is_continuation, is_final
            )
    ...
    def _build_page(
        ...
        for i, image in enumerate(images):
            ...
            # Step label
            step_num = image.step_number or (
                page_idx * self.config.max_steps_per_page + i + 1
            )
...
class EnhancedLaTeXFigureGenerator:
    ...
    def generate(self, spec: FigureSpec, validate: bool = True) -> GeneratedFigure:
        """
    ...
    def generate_single(
        self, image_path: str, caption: str, label: str = None, width: float = 0.8
    ) -> GeneratedFigure:
    ...
    def generate_procedural_sequence(
        self, steps: list[dict[str, Any]], procedure_title: str, label: str = None
    ) -> GeneratedFigure:
        """
...
class BatchFigureGenerator:
    ...
    def generate_all(self, specs: list[FigureSpec]) -> list[GeneratedFigure]:
        """Generate LaTeX for all specifications."""
        return [self.generator.generate(spec) for spec in specs]

    def generate_chapter(self, specs: list[FigureSpec], chapter_title: str) -> str:
    ...
    def export_to_file(self, specs: list[FigureSpec], output_path: str):
