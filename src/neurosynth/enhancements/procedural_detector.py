...
from .config import NeuroSynthEnhancedConfig, NeurosurgicalKeywords
...
class StepNumberExtractor:
    ...
    def extract(self, text: str) -> Optional[Tuple[int, SequenceType, float]]:
        """
...
class ProcedureKeywordDetector:
    ...
    def __init__(self, config: NeuroSynthEnhancedConfig = None):
        ...
        self.procedure_keywords = set(kw.lower() for kw in keywords.procedure)
        self.anatomy_keywords = set(kw.lower() for kw in keywords.anatomy)
        
        # Additional sequence-specific keywords
        self.sequence_indicators = {
            'first', 'second', 'third', 'fourth', 'fifth',
            'initial', 'final', 'next', 'then', 'subsequently',
            'followed by', 'after', 'before', 'during',
            'beginning', 'completion', 'completed',
        }
...
class ProceduralSequenceDetector:
    ...
    def _group_into_sequences(
        ...
        numbered_steps = [e for e in elements if e.sequence_type == SequenceType.NUMBERED_STEPS]
    ...
    def _group_consecutive(
        ...
        for elem in sorted_elems[1:]:
            ...
            is_consecutive = (
                elem.sequence_number == last.sequence_number + 1 and
                abs(elem.page_number - last.page_number) <= 2
            )
    ...
    def _overlaps_with_existing(
        self,
        group: List[SequenceElement],
        existing: List[List[SequenceElement]]
    ) -> bool:
        """Check if a group overlaps with existing candidates."""
        group_ids = set(e.image_id for e in group)
        
        for existing_group in existing:
            existing_ids = set(e.image_id for e in existing_group)
    ...
    def _validate_sequences(
        self,
        candidates: List[List[SequenceElement]]
    ) -> List[ProceduralSequence]:
    ...
    def _sort_sequence_elements(self, sequence: ProceduralSequence):
        ...
        if sequence.sequence_type in (SequenceType.NUMBERED_STEPS, 
    ...
    def _calculate_sequence_metrics(self, sequence: ProceduralSequence):
        ...
        sequence.completeness = actual_count / expected_count if expected_count > 0 else 0
...
class SequenceValidator:
    ...
    def validate(self, sequence: ProceduralSequence) -> Dict[str, Any]:
        ...
        if sequence.completeness < 0.8:
            warnings.append(f"Sequence has gaps (completeness: {sequence.completeness:.0%})")
...
class SequenceExporter:
    ...
    def to_latex_sequence(
        ...
        for i, elem in enumerate(sequence.elements):
            lines.append(f"    \\begin{{subfigure}}[b]{{0.45\\textwidth}}")
            lines.append(f"        \\centering")
            lines.append(f"        \\includegraphics[width=\\textwidth]{{{image_path_prefix}{elem.image_id}}}")
