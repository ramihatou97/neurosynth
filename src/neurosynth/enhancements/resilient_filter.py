...
class ColorAnalyzer:
    ...
    def _analyze_with_pil(self, image_bytes: bytes) -> ColorAnalysisResult | None:
        ...
        unique = {(r // 16, g // 16, b // 16) for r, g, b in sampled}
    ...
    def _analyze_with_pymupdf(
        self, doc: "fitz.Document", xref: int
...
class SurgicalColorDetector:
    ...
    def detect(
        self, image_bytes: bytes, doc: "fitz.Document" = None, xref: int = 0
    ) -> float:
        """
        ...
        try:
            ...
            for row_idx in range(0, height, row_step):
                ...
                for col in range(width - 1):
                    if (
                        abs(samples[row_start + col] - samples[row_start + col + 1])
                        > 30
                    ):
                        edge_count += 1
...
class EnhancedMedicalClassifier:
    ...
    def classify(
        ...
        # Color analysis
        color_result = self.color_analyzer.analyze(
        ...
        is_grayscale = (
            color_result.grayscale_ratio > self.config.radiological_grayscale_ratio
        )
        ...
        # Icon rejection
        if (
            entropy < self.config.icon_max_entropy
            and area < self.config.icon_max_area
            and color_result.unique_colors < self.config.icon_max_colors
        ):
            return FilterResult(
                should_extract=False,
                confidence=0.0,
        ...
        return FilterResult(
            ...
            features=features,
            rejection_reason=(
                None if score >= self.config.min_confidence else "low_confidence"
            ),
        )
