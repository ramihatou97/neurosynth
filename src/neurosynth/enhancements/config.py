...
class CaptionConfidenceConfig:
    ...
    def validate(self) -> bool:
        total = (
            ...
            + self.formatting_weight
            + self.length_weight
        )
        if not (0.95 <= total <= 1.05):
            raise ValueError(
                f"Caption confidence weights must sum to ~1.0, got {total:.2f}"
            )
        return True
...
class ImageFilterConfig:
    ...
    surgical_colors: dict[str, tuple[tuple[int, int, int], tuple[int, int, int]]] = (
        field(
            default_factory=lambda: {
...
class CaptionDetectionConfig:
    ...
    # Patterns
    figure_patterns: list[str] = field(
        default_factory=lambda: [
    ...
    cross_reference_patterns: list[str] = field(
        default_factory=lambda: [
...
class VectorGraphicsConfig:
    ...
    flowchart_keywords: list[str] = field(
        default_factory=lambda: [
            "algorithm",
            "flowchart",
            "decision",
    ...
    diagram_keywords: list[str] = field(
        default_factory=lambda: [
            "anatomy",
            "schematic",
            "illustration",
...
class LaTeXConfig:
    ...
    size_presets: dict[str, float] = field(
        default_factory=lambda: {
            "full": 1.0,
            "large": 0.8,
            "medium": 0.6,
...
class NeuroSynthEnhancedConfig:
    ...
    association_weights: AssociationWeightConfig = field(
        default_factory=AssociationWeightConfig
    )
    caption_confidence: CaptionConfidenceConfig = field(
        default_factory=CaptionConfidenceConfig
    )
    image_filter: ImageFilterConfig = field(default_factory=ImageFilterConfig)
    caption_detection: CaptionDetectionConfig = field(
        default_factory=CaptionDetectionConfig
    )
    ...
    def print_weights_summary(self):
        ...
        total_kw = (
            kw.anatomy + kw.pathology + kw.procedure + kw.imaging + kw.instruments
        )
