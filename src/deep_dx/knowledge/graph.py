...
class DeepDxKnowledgeGraph:
    ...
    def __init__(self):
        ...
        try:
            ...
            pwd = (
                self.settings.neo4j_password or "password"
            )  # usually user needs to set this
            ...
            print(
                "   -> Graph operations will run in 'Dry Run' mode (Extraction only)."
            )
...
"""
    ...
    def ingest_chunk(self, chunk_id: str, text: str, source: str):
        ...
        print(
            f"   -> Extracted {len(graph_data['entities'])} entities, {len(graph_data['relationships'])} rels."
        )
        ...
        with self.driver.session() as session:
            ...
            for ent in graph_data["entities"]:
                if not ent.get("name") or not ent.get("label"):
                    continue
                ...
                """
                session.run(
                    query, name=ent["name"], label=ent["label"], chunk_id=chunk_id
                )
