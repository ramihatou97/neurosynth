...
class RaptorIndexer:
    ...
    def cluster_embeddings(
        self, embeddings: np.ndarray, max_cluster_size: int = 10
    ) -> list[list[int]]:
        """
...
SUMMARY:"""
    ...
    def build_tree(
        self, chunks: list[dict], distinct_layer_name: str = "raptor"
    ) -> list[dict]:
        """
        ...
        while len(current_layer_nodes) > 5:  # Stop when we have few summary nodes
            print(
                f"🦖 RAPTOR Level {level}: Processing {len(current_layer_nodes)} nodes..."
            )
            ...
            for i, cluster_indices in enumerate(clusters):
                # Concatenate texts
                cluster_texts = [
                    current_layer_nodes[idx]["text"] for idx in cluster_indices
                ]
                full_text = "\n---\n".join(cluster_texts)

                print(
                    f"   -> Summarizing Cluster {i+1}/{len(clusters)} ({len(cluster_texts)} chunks)..."
                )
