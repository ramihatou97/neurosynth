...
from models import Chunk, ChunkType, SourceMetadata, DocumentType, Specialty
...
class DeepDxIngestor:
    ...
    def ingest_pdf(self, pdf_path: Path):
        ...
        if "spine" in fname: specialty = Specialty.SPINE
        elif "tumor" in fname or "glioma" in fname: specialty = Specialty.TUMOR
        elif "anatomy" in fname: specialty = Specialty.ANATOMY
        ...
        # 5. Embed (Batch)
        try:
            embeddings = self.ai_client.get_embeddings(texts_to_embed) # Handles batching internally
