...
class ColBERTClient:
    ...
    def _ensure_container_running(self):
        ...
        subprocess.run(
            [
                ...
                self.container_name,
                "-v",
                f"{project_root}:/app",  # Mount current dir to /app
                "colbert_radical",  # The Image Name
                "tail",
                "-f",
                "/dev/null",  # Keep alive command
            ],
            check=True,
        )
    ...
    def search(self, query: str, k: int = 20) -> list[dict]:
        ...
        cmd = [
            ...
            self.container_name,
            "python",
            "/app/src/deep_dx/docker_scripts/search.py",
            "--input",
            "/app/data/colbert_io/input.json",
            "--output",
            "/app/data/colbert_io/output.json",
        ]
    ...
    def rerank(self, query: str, documents: list[str], k: int = 20) -> list[dict]:
        ...
        cmd = [
            ...
            self.container_name,
            "python",
            "/app/src/deep_dx/docker_scripts/rerank.py",
            "--input",
            "/app/data/colbert_io/rerank_input.json",
            "--output",
            "/app/data/colbert_io/rerank_output.json",
        ]
    ...
    def index(self, index_name: str, documents: list[str]):
        ...
        cmd = [
            ...
            self.container_name,
            "python",
            "/app/src/deep_dx/docker_scripts/index.py",
            "--input",
            "/app/data/colbert_io/index_input.json",
            "--index_name",
            index_name,
        ]
