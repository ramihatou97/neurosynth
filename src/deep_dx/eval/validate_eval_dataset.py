...
def validate_dataset(filepath: Path) -> bool:
    ...
    # Print header
    print(f"\n{'='*60}")
    print("DEEP-DX EVALUATION DATASET VALIDATION")
    print(f"{'='*60}\n")
    ...
    print(
        f"  Adversarial queries: {adversarial_count} (target: ≥10) {adversarial_status}"
    )
    ...
    short_ground_truth = [
        q["id"] for q in queries if len(q.get("ground_truth", "")) < 100
    ]
    if short_ground_truth:
        print(
            f"  ⚠️  Short ground truth (<100 chars): {', '.join(short_ground_truth[:5])}"
        )
        if len(short_ground_truth) > 5:
            print(f"     ... and {len(short_ground_truth) - 5} more")
        issues.append("short_ground_truth")
    else:
        print("  ✓  All ground truth answers are detailed")
    ...
    if vague_queries:
        ...
        print("  ✓  All queries are specific")
    ...
    if invalid_ids:
        print(
            f"  ⚠️  Invalid ID format (should be Q001, Q002, ...): {', '.join(invalid_ids[:5])}"
        )
        if len(invalid_ids) > 5:
            print(f"     ... and {len(invalid_ids) - 5} more")
    else:
        print("  ✓  All IDs follow Q### format")

    print(f"\n{'='*60}\n")

    # Final verdict
    critical_issues = (
        "duplicate_ids" in issues
        or "missing_id" in issues
        or "missing_query" in issues
        or "missing_ground_truth" in issues
        or safety_pct < 25
        or total < 80
        or not all_types_ok
    )
    ...
    minor_issues = (
        "short_ground_truth" in issues
        or adversarial_count < 10
        or abs(total - 100) > 10
    )
    ...
    if critical_issues:
        ...
        if safety_pct < 25:
            print(
                f"  • Increase safety-critical queries (currently {safety_pct:.1f}%, need ≥30%)"
            )
        if total < 80:
        ...
        if "short_ground_truth" in issues:
            print(
                f"  • Expand ground truth answers for {len(short_ground_truth)} queries"
            )
        if adversarial_count < 10:
            print(
                f"  • Add more adversarial queries (currently {adversarial_count}, target ≥10)"
            )
        if abs(total - 100) > 10:
