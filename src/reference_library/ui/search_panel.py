...
class SearchPanel(ctk.CTkFrame):
    ...
    def _setup_ui(self):
        ...
        self.strategy_selector = ctk.CTkOptionMenu(
            search_frame,
            values=["🎯 Strict", "⚖️ Standard", "📚 Study Mode"],
    ...
    def _on_exam_mode_toggle(self):
        ...
        self._show_strategy_hint(
            f"Exam Mode {status} - {'High-yield topics boosted' if enabled else 'Standard ranking'}"
        )
    ...
    # ==================== Gap Warning Methods (Phase 4) ====================

    def show_gap_warnings(self, warnings: list[str]):
        """Display gap detection warnings with color coding.

        Args:
            warnings: List of warning strings from PrecisionRetrievalResult
        """
        if not warnings:
            return

        # Show first (most severe) warning
        first_warning = warnings[0]

        # Color code by severity emoji
        if first_warning.startswith("⚠️"):
            color = "#e67e22"  # Orange (CRITICAL/HIGH)
        elif first_warning.startswith("ℹ️"):
            color = "#3498db"  # Blue (MEDIUM)
        elif first_warning.startswith("💡"):
            color = "#95a5a6"  # Gray (LOW/tips)
        else:
            color = "#3498db"  # Default blue

        # Create gap hint label if not exists
        if not hasattr(self, "_gap_hint"):
            self._gap_hint = ctk.CTkLabel(
                self, text="", font=FONTS["small"], text_color=color
            )

        self._gap_hint.configure(text=first_warning, text_color=color)
        self._gap_hint.pack(fill="x", padx=PADDING["medium"], pady=(0, 4))

        # Auto-hide after 5 seconds (longer than strategy hint)
        if hasattr(self, "_gap_hint_after_id"):
            self.after_cancel(self._gap_hint_after_id)
        self._gap_hint_after_id = self.after(5000, self._hide_gap_hint)

    def _hide_gap_hint(self):
        """Hide gap warning hint."""
        if hasattr(self, "_gap_hint"):
            self._gap_hint.pack_forget()

    # ==================== End Gap Warning Methods ====================
