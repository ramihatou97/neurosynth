...
def _extract_pages_from_pdf(
    ...
    try:
        ...
        is_enhanced = hasattr(results[0], "matched_pages") and hasattr(
            results[0], "matched_sections"
        )
        ...
        for page_num in pages:
            ...
            try:
                ...
                logger.warning(
                    "Could not insert page %d from %s: %s", page_num, pdf_path.name, e
                )
        ...
        safe_name = _safe_filename(
            first_result.chapter_title or first_result.book_title
        )
        page_range = (
            f"p{successfully_inserted_pages[0]}-{successfully_inserted_pages[-1]}"
            if len(successfully_inserted_pages) > 1
            else f"p{successfully_inserted_pages[0]}"
        )
        ...
        return ExtractedSource(
            ...
            matched_sections=[
                s.title for s in getattr(first_result, "matched_sections", [])
            ],
            intent=(
                getattr(first_result, "match_type", "").name
                if hasattr(first_result, "match_type")
                else ""
            ),
        )
...
def _compute_figure_summary(sources: list[ExtractedSource]) -> dict:
    ...
    return {"total_figures": total_figures, "figures_by_type": figures_by_type}
