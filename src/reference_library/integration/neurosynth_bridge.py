...
def _sanitize_topic(topic: str) -> str:
    ...
    if not re.match(r"^[\w\s\-\.\,\:\;\(\)\'\"]+$", topic, re.UNICODE):
        raise ValueError(
            "Invalid topic name: contains unsafe characters. "
...
from ..export.page_extractor import (
    ExtractedSource,
...
class NeuroSynthBridge:
    ...
    def synthesize(
        ...
        if not results:
            ...
            if self.pdf_searcher and search_query:
                ...
                search_results = self.pdf_searcher.search_library_chapters(
                    query=search_query, mode=search_mode
                )
                ...
                return SynthesisResult(
                    success=False, error="No results selected for synthesis"
                )
        ...
        try:
            ...
            return SynthesisResult(success=False, error=f"Failed to extract pages: {e}")

        if not extracted:
            return SynthesisResult(
                success=False, error="No pages could be extracted from selected results"
            )
        ...
        if not detected_intent and results:
            ...
            if (
                hasattr(first_result, "matched_sections")
                and first_result.matched_sections
            ):
    ...
    def _collect_project_images(
        self, extracted: list[ExtractedSource], images_dir: Path
    ) -> int:
    ...
    def _run_neurosynth(
        ...
        cmd.extend(
            ["run", topic, "--sources", str(sources_dir), "--output", str(output_path)]
        )
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                if output_path.exists():
                    ...
                    if tex_path.exists():
                        ...
                        return SynthesisResult(
                            success=True, output_path=tex_path, log=log
                        )
                    return SynthesisResult(
                        success=False,
            ...
            return SynthesisResult(success=False, error=f"Unexpected error: {e}")
    ...
    def _get_neurosynth_command(self) -> list[str]:
        ...
        if self.neurosynth_path:
            ...
            python_bin = (
                self.neurosynth_venv / "bin" / "python"
                if self.neurosynth_venv
                else "python"
            )
