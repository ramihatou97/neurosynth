...
class MatchConfidence(Enum):
    ...
    MEDIUM = auto()  # Strong keyword match but maybe inline (e.g., "SURGICAL TECHNIQUE")
...
class SectionDetector:
    ...
    PATTERNS: Dict[str, List[Tuple[str, MatchConfidence]]] = {
        "Technique": [
            (r'^\s*(?:[0-9]+(?:\.[0-9]+)*\.?|[IVX]+\.)?\s*SURGICAL\s+TECHNIQUE\s*$', MatchConfidence.HIGH),
            (r'^\s*(?:[0-9]+(?:\.[0-9]+)*\.?|[IVX]+\.)?\s*OPERATIVE\s+TECHNIQUE\s*$', MatchConfidence.HIGH),
            (r'^\s*(?:[0-9]+(?:\.[0-9]+)*\.?|[IVX]+\.)?\s*Surgical\s+Technique\s*$', MatchConfidence.HIGH),
            (r'^\s*(?:[0-9]+(?:\.[0-9]+)*\.?|[IVX]+\.)?\s*Operative\s+Procedure\s*$', MatchConfidence.HIGH),
            (r'^\s*SURGICAL\s+STEPS\s*$', MatchConfidence.MEDIUM),
            (r'^\s*OPERATIVE\s+NUANCES\s*$', MatchConfidence.MEDIUM),
        ],
        "Complication": [
            (r'^\s*(?:[0-9]+(?:\.[0-9]+)*\.?|[IVX]+\.)?\s*COMPLICATIONS\s*$', MatchConfidence.HIGH),
            (r'^\s*(?:[0-9]+(?:\.[0-9]+)*\.?|[IVX]+\.)?\s*Complications\s*$', MatchConfidence.HIGH),
            (r'^\s*COMPLICATION\s+AVOIDANCE\s*$', MatchConfidence.MEDIUM),
            (r'^\s*(?:[0-9]+(?:\.[0-9]+)*\.?|[IVX]+\.)?\s*Complication\s+Avoidance\s*$', MatchConfidence.MEDIUM),
            (r'^\s*POSTOPERATIVE\s+MANAGEMENT\s*$', MatchConfidence.MEDIUM),
            (r'^\s*(?:[0-9]+(?:\.[0-9]+)*\.?|[IVX]+\.)?\s*Postoperative\s+Management\s*$', MatchConfidence.MEDIUM),
        ],
        "Anatomy": [
            (r'^\s*(?:[0-9]+(?:\.[0-9]+)*\.?|[IVX]+\.)?\s*SURGICAL\s+ANATOMY\s*$', MatchConfidence.HIGH),
            (r'^\s*(?:[0-9]+(?:\.[0-9]+)*\.?|[IVX]+\.)?\s*RELEVANT\s+ANATOMY\s*$', MatchConfidence.HIGH),
            (r'^\s*(?:[0-9]+(?:\.[0-9]+)*\.?|[IVX]+\.)?\s*Surgical\s+Anatomy\s*$', MatchConfidence.HIGH),
            (r'^\s*ANATOMICAL\s+CONSIDERATIONS\s*$', MatchConfidence.MEDIUM),
        ],
        "Indication": [
            (r'^\s*(?:[0-9]+(?:\.[0-9]+)*\.?|[IVX]+\.)?\s*INDICATIONS\s*$', MatchConfidence.HIGH),
            (r'^\s*(?:[0-9]+(?:\.[0-9]+)*\.?|[IVX]+\.)?\s*Indications\s*$', MatchConfidence.HIGH),
            (r'^\s*(?:[0-9]+(?:\.[0-9]+)*\.?|[IVX]+\.)?\s*PATIENT\s+SELECTION\s*$', MatchConfidence.HIGH),
            (r'^\s*(?:[0-9]+(?:\.[0-9]+)*\.?|[IVX]+\.)?\s*Patient\s+Selection\s*$', MatchConfidence.HIGH),
            (r'^\s*CLINICAL\s+PRESENTATION\s*$', MatchConfidence.MEDIUM),
            (r'^\s*(?:[0-9]+(?:\.[0-9]+)*\.?|[IVX]+\.)?\s*Clinical\s+Presentation\s*$', MatchConfidence.MEDIUM),
        ],
        "Outcome": [
            (r'^\s*(?:[0-9]+(?:\.[0-9]+)*\.?|[IVX]+\.)?\s*OUTCOMES\s*$', MatchConfidence.HIGH),
            (r'^\s*(?:[0-9]+(?:\.[0-9]+)*\.?|[IVX]+\.)?\s*Outcomes\s*$', MatchConfidence.HIGH),
            (r'^\s*(?:[0-9]+(?:\.[0-9]+)*\.?|[IVX]+\.)?\s*RESULTS\s*$', MatchConfidence.HIGH),
            (r'^\s*(?:[0-9]+(?:\.[0-9]+)*\.?|[IVX]+\.)?\s*Results\s*$', MatchConfidence.HIGH),
            (r'^\s*PROGNOSIS\s*$', MatchConfidence.MEDIUM),
            (r'^\s*(?:[0-9]+(?:\.[0-9]+)*\.?|[IVX]+\.)?\s*Prognosis\s*$', MatchConfidence.MEDIUM),
        ]
    }
    ...
    def get_safe_extraction_window(
        ...
        sorted_sections = sorted(all_sections, key=lambda s: (s.page_num, s.start_offset))
        ...
        for section in sorted_sections:
            ...
            if (section.page_num == target_section.page_num and 
                section.start_offset == target_section.start_offset):
                found_current = True
    ...
    def create_fallback_section(self, keyword_page: int, total_pages: int) -> Tuple[int, int]:
