...
if "PYTHONPATH" not in os.environ:
    ...
    os.environ["PYTHONPATH"] = str(repo_root / "src") + os.pathsep + os.environ["PYTHONPATH"]
...
class DockerBridge(NeuroSynthBridge):
    ...
    def synthesize(
        ...
        run_id = "".join(c for c in topic.lower() if c.isalnum() or c in (' ', '_', '-')).replace(" ", "_")[:50]
    ...
    def _run_neurosynth(self, topic, sources_dir, output_path, manifest_path, on_progress=None):
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                if local_output_path.exists():
                    ...
                    return SynthesisResult(success=True, output_path=output_path, log=log)
                ...
                return SynthesisResult(success=False, error="Output file missing", log=log)
            else:
                return SynthesisResult(success=False, error=f"Exit code {process.returncode}", log=log)
