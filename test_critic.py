...
def main():
    ...
    chunks = [
        ...
        {
            "id": 1,
            "text": "The standard of care for glioblastoma includes maximal safe surgical resection followed by radiation therapy and concurrent temozolomide chemotherapy (Stupp protocol).",
        },
        # Irrelevant
        {
            "id": 2,
            "text": "The reception desk is located on the first floor near the elevating lifts. Parking validation is available for patients.",
        },
        # Semi-relevant
        {
            "id": 3,
            "text": "Brain tumors can be benign or malignant. Diagnosis typically involves MRI imaging.",
        },
    ]
    ...
    print(
        f"  Safe Answer Result: {res_safe['safe']} (Risk: {res_safe.get('risk_level')})"
    )
    ...
    print(
        f"  Unsafe Answer Result: {res_unsafe['safe']} (Risk: {res_unsafe.get('risk_level')})"
    )
