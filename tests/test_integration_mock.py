...
def mock_parser():
    with patch("neurosynth.parsers.ParserFactory.get_parser") as mock_factory:
        ...
        doc.add_chunk("This is a test document about lumbar discectomy. It involves removing a herniated disc.")
...
def mock_llm_clients():
    with patch("neurosynth.dedup.embeddings.VoyageClient") as mock_voyage, \
        ...
        voyage_instance.embed_texts = AsyncMock(return_value=[[0.1] * 1024]) # Mock embedding
        ...
        claude_instance.generate_outline = AsyncMock(return_value=[
            {"title": "Introduction", "level": 1, "description": "Intro"},
            {"title": "Technique", "level": 1, "description": "Steps"}
        ])
        claude_instance.synthesize_section = AsyncMock(return_value="Synthesized section content.")
        claude_instance.generate_abstract = AsyncMock(return_value="Abstract.")
        claude_instance.extract_keywords = AsyncMock(return_value=["keyword1", "keyword2"])
