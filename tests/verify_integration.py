...
def verify_integration():
    ...
    # Setup paths
    library_path = Path(
        "/Users/ramihatoum/neurosynth/real_synthesis_output/temp_library"
    )
    ...
    db = Database(db_path)
    LibraryScanner(library_path, db)
    SemanticSearcher(db)
    get_master_index()

    pdf_searcher = PDFSearcher(library_path=library_path, database=db)

    # Initialize Bridge
    bridge = NeuroSynthBridge(database=db, pdf_searcher=pdf_searcher)
