...
def chunk_text(
    ...
    while start < text_len:
        ...
        if len(chunk_text_str) > 50:  # Minimum viable chunk
...
class LibraryToDeepDxBridge:
    ...
    def connect(self) -> bool:
        ...
        print(f"OK Source: {self.source_db_path}")
        ...
        print(f"OK Target: {self.target_db_path}")
        ...
        if not self.skip_qdrant:
            try:
                ...
                print(f"OK Qdrant: {self.qdrant_url}")
            ...
            print("SKIP Qdrant: SKIPPED (--skip-qdrant flag)")

        # Voyage AI client
        self._voyage = VoyageClient()
        print("OK Voyage AI: Ready")
    ...
    async def push_to_qdrant(self, chunks: list[Chunk], source: SourceMetadata) -> int:
        ...
        for chunk in chunks:
            ...
            points.append(
                models.PointStruct(id=point_id, vector=chunk.embedding, payload=payload)
            )
        ...
        for i in range(0, len(points), QDRANT_BATCH_SIZE):
            ...
            try:
                ...
                print(f"  WARN Qdrant batch failed: {e}")
    ...
    async def run(self, limit: int = 0, force: bool = False):
        ...
        if total_files == 0:
            print("ERROR: No files with cached text found!")
            return
        ...
        for idx, file_info in enumerate(files, 1):
            ...
            try:
                ...
                if not chunks:
                    print("  SKIP No chunks generated (empty text)")
                    continue

                print(
                    f"  DOC {len(chunks)} chunks from {file_info['cached_pages']} pages"
                )

                # Generate embeddings
                embedded = await self.embed_chunks(chunks)
                print(f"  EMB {embedded} embeddings generated")
                ...
                if not self.skip_qdrant:
                    ...
                    print(f"  VEC {pushed} vectors pushed to Qdrant")
    ...
    def _print_summary(self):
        ...
        if self.stats["errors"]:
            print(f"\nWARN {len(self.stats['errors'])} errors occurred:")
